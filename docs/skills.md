...
Skills are installed via the `oh-my-oc` installer or manually with `npx skills add`.
...
Control which skills each agent can use in `~/.config/opencode/oh-my-oc.json` (or `.jsonc`):
