...
/** Individual agent descriptions, keyed by agent name. */
...
export const ENGRAM_LITE_PROTOCOL = `<Memory>
...
Only for high-value information:
- Architecture or design decisions
- Bug fixes with non-obvious root cause
- Non-obvious codebase discoveries or gotchas
- Patterns or conventions established

Format: title (verb + what), type (bugfix|decision|architecture|discovery|pattern), content with what/why/where.
Use \`topic_key\` for evolving topics (e.g. \`architecture/auth-model\`).

### Session close
Call \`mem_session_summary\` with: goal, accomplished items, next steps, relevant files.

### After compaction
If you see a compaction message: call \`mem_session_summary\` first, then \`mem_context\` to recover.

</Memory>`;

// ─── Core agents that are always included ────────────────────────────────────
...
/** Options that control which sections are included in the orchestrator prompt. */
 ...
 * Only includes agent descriptions and protocol sections that are active,
 * reducing token cost per turn.
 */
export function buildOrchestratorPrompt(
  options: OrchestratorOptions = {},
): string {
  // Build agents section with only active agents
  const agentBlocks = CORE_AGENTS.map((name) => AGENT_DESCRIPTIONS[name]);

  if (options.councilEnabled) {
    agentBlocks.push(AGENT_DESCRIPTIONS.council);
  }

  const agentsSection = `<Agents>\n\n${agentBlocks.join('\n\n')}\n\n</Agents>`;

  const sections = [ROLE, agentsSection, WORKFLOW, COMMUNICATION];

  if (options.engramEnabled) {
    sections.push(ENGRAM_LITE_PROTOCOL);
  }
...
export function createOrchestratorAgent(
  ...
  const basePrompt = buildOrchestratorPrompt(options);
